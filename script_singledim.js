...
const targetArea = document.getElementById("targetArea");  // NEW
...
function lockToBar(el, rawX, barRect) {
  const clampedX = Math.max(
    0,
    Math.min(rawX, barRect.width - el.offsetWidth)              // keep inside bar
  );
  const offsetY = -(el.offsetHeight - barRect.height) / 2;
...
// The drop event listener will need slight modifications
targetArea.addEventListener("drop", function (e) {
  e.preventDefault();

  const barRect  = dropZone.getBoundingClientRect();   // 20-px bar
  const rawX     = e.clientX - barRect.left - offsetX; // x relative to bar
  const clampedX = Math.max(0,
    Math.min(rawX, barRect.width - draggedElement.offsetWidth)
  );

  /* ===== New icon from the sound panel =================================== */
  if (!dropZone.contains(draggedElement)) {
    const clone = draggedElement.cloneNode(true);
    clone.style.backgroundColor = draggedElement.dataset.color;
    ...
    lockToBar(clone, clampedX, barRect);                // <- snap onto bar

    /* remove the top-row copy and advance to next sound (unchanged)… */
  ...
  /* ===== Icon already in the bar – move horizontally only ================ */
  } else {
    lockToBar(draggedElement, clampedX, barRect);
  }
});
...
function exportCSV() {
  // Prepare CSV header
  const headers = [
    "sound", "x_normalized", "color", "order", 
